...
export default function ExamPage() {
    ...
    const load = async () => {
      ...
      try {
        ...
        const totalTimeMinutes = Number(
          payload?.total_time ?? payload?.data?.total_time ?? payload?.time ?? 0
        );
  ...
  if (fetchError)
    return <div className="card p-6 text-red-600">{fetchError}</div>;
  if (!questions.length)
    return <div className="card p-6">No questions found</div>;
      ...
      <div className="p-6 md:p-5">
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="flex-1">
            <div className="flex items-center justify-between">
              <span className="text-base text-[#1C3141] mb-2 font-medium">
            ...
            <div className="space-y-3">
              {currentQuestion.options.map((opt, index) => (
                ...
                >
                  ...
                  <input
                    ...
                    onChange={() =>
                      selectOption(currentQuestion.question_id, opt.id)
                    }
                  />
                </label>
              ))}
              ...
              <div className="flex flex-wrap gap-3 mt-4 sm:flex-row flex-col">
                ...
                <button
                  ...
                  className="px-4 py-2 rounded border flex-1 cursor-pointer hover:bg-gray-300"
          ...
          <div className="w-full lg:w-1/3 pr-5 lg:px-6 lg:border-l border-gray-300">
            <div className="flex justify-between items-center mb-3">
              ...
              <div className="flex items-center gap-2 text-sm font-medium">
                ...
                <span className="bg-gray-900 text-white rounded-lg px-3 py-0.5">
            ...
            <div className="grid grid-cols-10 gap-2 mb-5">
              {questions.map((q, idx) => {
                ...
                if (!answers[q.question_id])
                  statusClass = "bg-gray-100 text-slate-800"; // not attended
                if (marked.has(q.question_id))
                  statusClass = "bg-purple-700 text-white"; // marked
                return (
                  <button
                    key={q.question_id}
        ...
        >
          <div className="space-y-4">
            <p>
              {currentQuestion.comprehension ?? "No extra paragraph available."}
            </p>
          </div>
        </Modal>
